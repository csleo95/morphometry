...
def get_vertex_df(surf_dir, features, parcellation):
    ...
    for hemi in ['lh','rh']:
        ...
        if hemi == 'lh':
            #print('Loading left hemisphere data:') #adjusted

            for i, lh_feature_loc in enumerate(lh_feature_locs):
                #print(lh_feature_loc) #adjusted
                #check for mgh/mgz format vs regular curv files
                if lh_feature_loc.endswith('mgh') or lh_feature_loc.endswith('mgz'):
                    name = os.path.basename(lh_feature_loc).split('.')[1] #adjusted
                    hemi_data_dict['Feature_' + name] = load(lh_feature_loc).get_fdata().flatten() #adjusted
                    #hemi_data_dict['Feature_' + str(i)] = load(lh_feature_loc).get_fdata().flatten() #adjusted
                else:
                    name = os.path.basename(lh_feature_loc).split('.')[1]
                    hemi_data_dict['Feature_' + name] = read_morph_data(lh_feature_loc)
                    #hemi_data_dict['Feature_' + str(i)] = read_morph_data(lh_feature_loc)

        elif hemi == 'rh':
            #print('Loading right hemisphere data:') #adjusted

            for i, rh_feature_loc in enumerate(rh_feature_locs):

                if rh_feature_loc.endswith('mgh') or rh_feature_loc.endswith('mgz'):
                    name = os.path.basename(rh_feature_loc).split('.')[1]
                    hemi_data_dict['Feature_' + name] = load(rh_feature_loc).get_fdata().flatten() #adjusted
                    #hemi_data_dict['Feature_' + str(i)] = load(rh_feature_loc).get_fdata().flatten() #adjusted
                else:
                    name = os.path.basename(rh_feature_loc).split('.')[1] #adjusted
                    hemi_data_dict['Feature_' + name] = read_morph_data(rh_feature_loc) #adjusted
                    #hemi_data_dict['Feature_' + str(i)] = read_morph_data(rh_feature_loc) #adjusted

        used_features = list(hemi_data_dict.keys())
        #print(used_features) #adjusted
